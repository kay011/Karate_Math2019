...
%     if(point(i,4) == 0)
%         point_0(end+1,:) = point(i,:);
%     else
%         point_1(end+1,:) = point(i,:);
%     end
% end

% 加载选取的校准点
arr_res = xlsread('2.xls');
%% 绘图部分
% 0 代表水平误差校准点，用黄色表示
% 1 代表垂直误差校准点，用蓝色表示
figure();
...
hold on;
scatter3(arr_res(:,1),arr_res(:,2),arr_res(:,3),'o');
hold on;
%% 航迹图
...
zlabel('Z轴');
%% 图例
legend('垂直校准点','水平校准点','起始点','终点','所选航迹点','航线路径');
